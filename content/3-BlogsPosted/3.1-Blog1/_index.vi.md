...
Khi phát triển một ứng dụng web, việc xây dựng chức năng đăng nhập thôi là chưa đủ. Điều quan trọng hơn là đảm bảo chỉ những người dùng đã được xác thực mới có thể truy cập vào các API và tài nguyên của hệ thống. Nếu không có cơ chế kiểm soát phù hợp, các API có thể bị truy cập trái phép, gây ảnh hưởng đến tính bảo mật của ứng dụng.
...
* Hệ thống chuyển hướng đến trang đăng nhập của Amazon Cognito.
* Sau khi đăng nhập thành công, Cognito cấp Access Token (hoặc JWT).
* Các yêu cầu gửi đến backend sẽ mang theo token này.
* Dịch vụ AWS hoặc backend sẽ kiểm tra token trước khi cho phép truy cập tài nguyên.
...
![Mô hình 1: Application Load Balancer kết hợp Amazon Cognito](/FCAJ-workshop-template/images/3-BlogsPosted/blog1/model-1-alb-cognito.png)
...
![Mô hình 2: Amazon API Gateway kết hợp Amazon Cognito](/FCAJ-workshop-template/images/3-BlogsPosted/blog1/model-2-api-gateway-cognito.png)
...
* Tài liệu Amazon Cognito.
* Tài liệu Application Load Balancer Authentication.
* Tài liệu Amazon API Gateway JWT Authorizer.
